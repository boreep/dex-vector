...
class SingleHandDetector:
    ...
    def detect(self, rgb):
        ...
        #关键点数据解析为数组再，减去手腕位置
        keypoint_3d_array = self.parse_keypoint_3d(keypoint_3d)#维度为[21:3]

        keypoint_3d_array = keypoint_3d_array - keypoint_3d_array[0:1, :]
        mediapipe_wrist_rot = self.estimate_frame_from_hand_points(keypoint_3d_array)#获得一个旋转矩阵，表示手腕坐标系
        joint_pos = keypoint_3d_array @ mediapipe_wrist_rot @ self.operator2mano
        return num_box, joint_pos, keypoint_2d, keypoint_3d_array, mediapipe_wrist_rot
    ...
    #x轴由手腕指向中指第一个关节，y轴为SVD计算得到平面的法向量，z轴由x和y叉乘得到
    @staticmethod
