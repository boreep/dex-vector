...
class Mimic3JointKinematicAdaptor(KinematicAdaptor):
    def __init__(
        self,
        robot: RobotWrapper,
        target_joint_names: List[str],
        source_joint_names: List[str],
        mimic_joint_names: List[str],
        multipliers1: List[float],
        multipliers2: List[float],
        multipliers3: List[float],
        offsets: List[float],
    ):
        super().__init__(robot, target_joint_names)

        self.multipliers1 = np.array(multipliers1)
        self.multipliers2 = np.array(multipliers2)
        self.multipliers3 = np.array(multipliers3)
        ...
        if len(union_set) > 0:
            raise ValueError(
                f"Mimic3 joint should not be one of the target joints.\n"
                f"Mimic3 joints: {mimic_joint_names}.\n"
                f"Target joints: {target_joint_names}\n"
                f"You need to specify the target joint names explicitly in your retargeting config"
                f" for robot with mimic joint constraints: {target_joint_names}"
            )
        ...
        # Dimension check
        len_source, len_mimic = self.idx_target2source.shape[0], self.idx_pin2mimic.shape[0]
        len_mul1,len_mul2,len_mul3, len_offset = self.multipliers1.shape[0], self.multipliers2.shape[0],self.multipliers3.shape[0],self.offsets.shape[0]
        if not (len_mimic == len_source == len_mul1==len_mul2==len_mul3 == len_offset):
            raise ValueError(
                f"Mimic joints setting dimension mismatch.\n"
                f"Source joints: {len_source}, mimic joints: {len_mimic}, multiplier1: {len_mul1}, multiplier2: {len_mul2},multiplier3: {len_mul3},offset: {len_offset}"
            )
        self.num_active_joints = len(robot.dof_joint_names) - len_mimic
    ...
    def forward_qpos(self, pin_qpos: np.ndarray) -> np.ndarray:
        # 获取索引对应位置的qpos值
        source_values = pin_qpos[self.idx_pin2source]

        mimic_qpos = (source_values * self.multipliers1
                    + source_values**2 * self.multipliers2 
                    + source_values**3 * self.multipliers3 
                    + self.offsets)
    ...
    def backward_jacobian(self, jacobian: np.ndarray) -> np.ndarray:
        target_jacobian = jacobian[..., self.idx_pin2target]
        mimic_joint_jacobian = jacobian[..., self.idx_pin2mimic] **3* self.multipliers3+ jacobian[..., self.idx_pin2mimic] **2* self.multipliers2 + jacobian[..., self.idx_pin2mimic] * self.multipliers1+ self.offsets
